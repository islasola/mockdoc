...
For this example, we are just using a 1 CU instance and using the Torchvision pre-trained Resnet50 model for embeddings. Let's get started!

## Installing the requirements

For this example, we are going to be using `pymilvus` to connect to use Zilliz Cloud, `torch` for running the embedding model, `torchvision` for the actual model and preprocessing, `gdown` to download the example dataset and `tqdm` for loading bars.
...
![Image search output](https://assets.zilliz.com/zillizCloudDocAssets/integrate_with_pytorch.png)